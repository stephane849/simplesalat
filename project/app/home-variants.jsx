...
function HomeLedger({ times, nowMin, prefs, hijri, dateStr }) {
...
function HomeCountdown({ times, nowMin, prefs, hijri, dateStr }) {
  const nk = nextPrayer(times, nowMin).key;
  const next = PRAYERS.find((p) => p.key === nk);
  const cd = countdown(times[nk], nowMin);
...
function DayArc({ times, nowMin, size = 'full', focusKey = null }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${vbH}`} width="100%" style={{ display: 'block', overflow: 'visible' }}>
      ...
      {/* focus ring on the selected prayer */}
      {focusKey &&
      <circle cx={fx(times[focusKey])} cy={fy(times[focusKey])} r="11" fill="none" stroke="var(--ink)" strokeWidth="2" />
      }
      {PRAYERS.map((p) => {
      ...
      {PRAYERS.map((p) => {const L = labelFor(p);const foc = p.key === focusKey;
        return <text key={p.key} x={L.x} y={L.ly} textAnchor={L.anchor}
...
/* ---------- C · ARC OF THE DAY — sun-path metaphor + prayer stepper ---------- */
function HomeArc({ times, nowMin, prefs, hijri, dateStr }) {
  ...
  return (
    <div className="view">
      ...
      <div className="scr-pad" style={{ paddingTop: 18, flex: '1 1 auto', display: 'flex', flexDirection: 'column', justifyContent: 'center' }} data-comment-anchor="8e6573c2eb-div-200-7">
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
          <span className="kicker">{isLive ? 'Next prayer' : `Prayer ${idx + 1} of ${dailies.length}`}</span>
          {!isLive &&
        ...
        <button className="btn" style={{ marginTop: 26, alignSelf: 'flex-start' }}
        onClick={() => setIdx((idx + 1) % dailies.length)}>
          Next prayer <Ico.right />
        </button>
      </div>
    </div>);
...
function HomeArcTimes({ times, nowMin, prefs, hijri, dateStr }) {
  const nk = nextPrayer(times, nowMin).key;
  const next = PRAYERS.find((p) => p.key === nk);
  const cd = countdown(times[nk], nowMin);
  return (
